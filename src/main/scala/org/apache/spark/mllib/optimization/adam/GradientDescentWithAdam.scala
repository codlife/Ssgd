package org.apache.spark.mllib.optimization.adam
...
/**
  ...
  */
class GradientDescentWithAdam(private var gradient: Gradient, private var updater: AdaGradUpdater)
  ...
  def optimize(data: RDD[(Double, Vector)], initialWeights: Vector): Vector = {
    val (weights, _) = GradientDescentWithAdam.runMiniBatchSGD(
      data,
      gradient,
      updater,
      stepSize,
...
object GradientDescentWithAdam extends Logging {
  /**
  ...
  def runMiniBatchSGD(
    ...
    val gHistory = BDV.zeros[Double](n)
    val gsquareHistory = BDV.zeros[Double](n)
    ...
    while (!converged && i <= numIterations) {
      ...
      if (miniBatchSize > 0) {
        ...
        for (k <- 0 until n) {
          ...
          gsquareHistory(k) = gsquareHistory(k) * 0.99 +  0.01* temp * temp
          gHistory(k) = gHistory(k) * 0.9 + 0.1 * temp

          val newg = gHistory(k)/0.1
          val newgs = gsquareHistory(k)/0.01

          //        print("this is eta"+gradientHistory(k))

          deltax(k) =  -stepSize * newg/ (sqrt(newgs) + 1e-8)
...
//        regVal = update._2
  ...
  GradientDescentWithAdam.runMiniBatchSGD(data, gradient, updater, stepSize, numIterations,

...
class AdaGradUpdater extends SquaredL2Updater {
  def  compute2(
               weightsOld: Vector,
               gradient: Vector,
               stepSize: Double,
...
//    val thisIterStepSize = stepSize / math.sqrt(iter)
    ...
    for (i <- 0 until n) {
      brzWeights(i) *= (1 -  stepSize*regParam * eta(i))
      brzWeights(i) -=  stepSize * gradient(i) * eta(i)
    }
    //brzAxpy(-thisIterStepSize, gradient.asBreeze, brzWeights)
    val norm = brzNorm(brzWeights, 2.0)
...
class AdaDeltaUpdater extends SquaredL2Updater {
  def  compute2(
                 weightsOld: Vector,
                 gradient: Vector,
                 stepSize: Double,

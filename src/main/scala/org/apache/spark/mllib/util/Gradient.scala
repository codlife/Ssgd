 ...
 * in multi-class spark.classification (it is also used in binary logistic regression).
 *
 ...
 * </blockquote></p>
 *
 * for K classes multiclass spark.classification problem.
 *
 ...
 *       $margins_i = x w_i$.
 *
 * For spark.optimization, we have to calculate the first derivative of the loss function, and
...
/**
 * :: DeveloperApi ::
 * Compute gradient and loss for a Hinge loss function, as used in SVM binary spark.classification.
 * See also the documentation for the precise formulation.
